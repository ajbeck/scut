...
The release workflow is manually dispatched with `releaseVersion` and an optional `targetSha`. When omitted, `targetSha` defaults to the current `origin/main` HEAD. It checks out `main`, verifies that the selected SHA matches the current `origin/main`, creates the immutable `vM.m.p` tag, and force-updates the movable `vM.m` and `vM` tags.
...
1. `tag-release`: validate stable SemVer, select the supplied full 40-character target SHA or the current `origin/main` HEAD, require it to match `origin/main`, create the exact tag, and update movable major/minor tags.
